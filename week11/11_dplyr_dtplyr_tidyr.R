...
# 11. Advanced data processing with dplyr, dtplyr, tidyr  #
...
medical <- suppressMessages(read_csv("medical.csv"))

# lets compare the efficiency of two csv reading functions

microbenchmark("read.csv" = {read.csv("medical.csv")},
               "read_csv" = {suppressMessages(read_csv("medical.csv"))},
               times = 10)
...
# - first ten observations and only as many columns
...
medical <- rename(medical,
                  GENDER = gender) # new name = old name
...
# we need to ignore missings in calculations (na.rm = TRUE)
...
# data.table package includes the fread() function
# which reads csv files very fast (usefull on large files)

medical_dt <- fread("medical.csv")

# it creates an object of data.table class
class(medical_dt)

# which is also a data frame, but not a tibble

# however, one can easily make the conversion

medical_dt <- tibble(medical_dt)

class(medical_dt)

# lets compare the efficiency of three approaches

microbenchmark("read.csv" = {read.csv("medical.csv")},
               "read_csv" = {suppressMessages(read_csv("medical.csv"))},
               "fread" = {fread("medical.csv")},
               times = 10)

# data.table is very efficient, but is syntax is much 
# more complex than in dplyr and not always intuitive

# but thanks to dtplyr package R will translate
# dplyr codes into data.table automatically
...
# Another package useful for preparing data
...
medical_states %>%
    summarise(min_share = min(SHARE_SOCIAL, na.rm = TRUE),
              max_share = max(SHARE_SOCIAL, na.rm = TRUE),
              n_all = n(),
              n_nonmissing = sum(!is.na(SHARE_SOCIAL))) %>%
    mutate(share_nonmissing = 100 * n_nonmissing/n_all) %>%
    arrange(share_nonmissing)
...
titanic <- suppressMessages(read_csv('titanic.csv'))
head(titanic)
class(titanic)
titanic
