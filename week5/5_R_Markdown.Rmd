...
<center><img src="5_img/newFile.png"></center>
...
In Markdown is implemented special syntax for equations, almost the same as in LaTeX and HTML (mathjax package). There are two ways of including equations in repotr, inline (single dollar signs): $A = \pi*r^{2}$, or as centered, separate sections (double dollar signs): $$A = \pi*r^{2}$$
...
<center><img src="5_WNE.png"></center>
```
...
If we want to finish tab section, we need to just put one hash symbol without any text after (otherwise this will be visible header)  and add paramwters: {.unlisted .unnumbered}
```
