...
### Exercise 1 
Write an if statement, which will return sum of a and b variables (if they are numbers), or their concatenation (if they are strings) and "error" in other cases.
...
Notation: `ifelse({condition}, {what if YES}, {what if NO})` Result is a vector with the same dimensions as the one in the condition.waring message wht is disturbing, condition is length >1 only first element will be used
...
produce as many values as the condition
...
c2) This second function should work with dataframe, thus
