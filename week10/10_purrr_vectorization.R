...
#             10. Purr package and vectorization          #
...
# Purrr package is a part of tidyverse project and provides set of functions for
# functional programming. More you can read about this package here:
...
# extract various information from this complex objected and store it in tabularized
...
map_df(got_chars, extract, c("name", "alive", "playedBy")) # does not work properly 
map_df(got_chars, extract, c("name", "alive", "playedBy"))[22,] # extracts only one information (here last actor to play)
ex[[20]] # three actors to play Eddard Stark here
...
# We can extract specific element of the field
got_chars %>%
...
got_chars %>% 
  map('aliases') %>% # extract aliases field and save it as list
  set_names(map_chr(got_chars, 'name')) %>% # change names of the list to the same as in originally list
  map(~ paste(.x, collapse = ", ")) # ~ is an alias of function, than .x is interpreted as placeholder where element of object should be placed
...
# doesn't work - we need some other tools for that - parallel map is a way to go
...
got_df$name %>% accumulate(`%+%`) # returns subsequent results of iterated cumulation values with given operator.
...
# Or with splice we can add something
...
data_tm <- readLines('1883_Theodore_Roosevelt.txt', encoding = 'UTF-8')  %>%  
  tibble() %>% 
...
system.time({
  for (i in 1:nrow(df)) {
    # print(i)
    if ((df[i, 1] + df[i, 2] + df[i, 3] + df[i, 4]) > 4) { #calculated and checked for each row separately
      df[i, 5] <- "more than 4"
    } else {
      df[i, 5] <- "4 or less" 
    }
...
system.time({
  ...
  for (i in 1:nrow(df)) { 
    if (condition[i]) { # now only check if value for this row is "true" - summation and condition check was done before for the whole vector at once
...
# 4.3. replicate()
# This function replicates result defined by user number of times
...
# 5.1. Read chapter Improving performance from book Advanced R by Hadley Wickham:
